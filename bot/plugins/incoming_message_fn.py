        ...
        try:
            ...
            if user.status == "kicked":
                await message.reply_text(
                    text="Sorry Sir, You are Banned to use me. Contact my owner",
                    parse_mode="markdown",
...
async def incoming_compress_message_f(bot, update):
    ...
    if update.reply_to_message is None:
        try:
            await bot.send_message(
                chat_id=update.chat.id,
                text="Reply to telegram media ‼️",
                reply_to_message_id=update.message_id
            )
        except:
            pass
    ...
    if len(update.command) > 1:
        try:
            if int(update.command[1]) <= 50 and int(update.command[1]) >= 20:
                target_crf = int(update.command[1])
            else:
                try:
                    await bot.send_message(
                        chat_id=update.chat.id,
                        text="CRF Value should be 20 to 50",
                        reply_to_message_id=update.message_id
                    )
                    return
                except:
    ...
    if os.path.exists(saved_file_path):
        ...
        if duration is None or bitrate is None:
            try:
                await sent_message.edit_text(
                    text="Getting video meta data failed!️"
                )
                chat_id = LOG_CHANNEL
                utc_now = datetime.datetime.utcnow()
                ist_now = utc_now + datetime.timedelta(minutes=30, hours=5)
        ...
        try:
            await sent_message.edit_text(
                text="Failed Downloaded path not exist!️"
            )
            chat_id = LOG_CHANNEL
            utc_now = datetime.datetime.utcnow()
            ist_now = utc_now + datetime.timedelta(minutes=30, hours=5)
